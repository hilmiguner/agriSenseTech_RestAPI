...
from database import insertUserMessage, getWeeds, deleteWeed
import base64
import os
...
@app.route("/delete_weed", methods=["POST"])
def delete_weed():
    data = request.json

    try:
        respond = deleteWeed(data["id"])
        if respond["status"] == 200:
            os.remove("img_weeds/" + data["image_path"])
        return jsonify(respond)
    except Exception as err:
        return jsonify({ 'status': 400, 'error': str(err) })
